...
export function EditIssueForm({ issue, open, onOpenChange }: EditIssueFormProps) {
  ...
  const onSubmit = async (values: z.infer<typeof updateIssueSchema>) => {
    setIsSubmitting(true);
    try {
      const result = await updateIssue(values);
      if (result.error) {
        throw new Error(result.error);
      }
      toast({
        title: 'Issue Updated',
        description: `Issue "${values.title}" has been successfully updated.`,
      });
      onOpenChange(false);
      router.refresh();
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="font-headline">Edit Issue</DialogTitle>
          <DialogDescription>
