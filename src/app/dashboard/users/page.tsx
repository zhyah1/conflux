  ...
  const handleInviteUser = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      ...
      toast({ 
          ...
          description: `An email has been sent to ${inviteEmail} with instructions to set up their password.`,
          duration: 10000,
      });
      setInviteEmail('');
      await fetchUsers();
        ...
        <CardContent>
          <form onSubmit={handleInviteUser} className="grid md:grid-cols-3 gap-4 items-end mb-6">
              ...
              <Button type="submit" disabled={isInviting}>
                {isInviting ? <Loader2 className="animate-spin" /> : 'Create and Invite User'}
              </Button>
          </form>
